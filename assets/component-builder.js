...
const initializeComponentBuilder = () => {
    ...
    const field = (definition,value,scope) => {
        ...
        if(definition.type==='select'||definition.type==='async_select'){const asyncAttr=definition.type==='async_select'?' data-searchable-select data-search-url="'+escape(definition.searchUrl||'')+'" data-placeholder="'+escape(definition.placeholder||t('Wybierz','Select'))+'"':'';return '<label>'+escape(definition.label)+'<select'+attr+asyncAttr+'>'+definition.options.map(option=>'<option value="'+escape(option[1])+'"'+(String(value)===String(option[1])?' selected':'')+'>'+escape(option[0])+'</option>').join('')+'</select>'+(definition.help?'<small>'+escape(definition.help)+'</small>':'')+'</label>';}
        if(definition.type==='multiselect'||definition.type==='async_multiselect'){const selected=new Set(String(value||'').split(',').map(item=>item.trim()).filter(Boolean));const asyncAttr=definition.type==='async_multiselect'?' data-searchable-select data-search-url="'+escape(definition.searchUrl||'')+'" data-placeholder="'+escape(definition.placeholder||t('Wybierz','Select'))+'"':'';return '<label>'+escape(definition.label)+'<select multiple size="'+escape(definition.size||6)+'"'+attr+asyncAttr+'>'+definition.options.map(option=>'<option value="'+escape(option[1])+'"'+(selected.has(String(option[1]))?' selected':'')+'>'+escape(option[0])+'</option>').join('')+'</select>'+(definition.help?'<small>'+escape(definition.help)+'</small>':'')+'</label>';}
    ...
    const synchronizeRichTextSources=()=>root.querySelectorAll('[data-rich-text-source]:not([hidden])').forEach(source=>{const component=findComponent(source.closest('[data-component-id]')?.dataset.componentId);if(component)component.data[source.dataset.field]=source.value;});
    const showEmbedPreview=(fieldElement,html)=>{const preview=fieldElement.querySelector('[data-rich-text-embed-preview]');preview.replaceChildren();const documentFragment=new DOMParser().parseFromString(html||'','text/html');const maps=[...documentFragment.querySelectorAll('iframe')].filter(frame=>{try{const url=new URL(frame.src);return url.protocol==='https:'&&url.hostname.endsWith('google.com')&&url.pathname.startsWith('/maps/embed');}catch{return false;}});maps.forEach(frame=>{const embed=document.createElement('iframe');embed.src=frame.src;embed.title=frame.title||t('Mapa Google','Google Map');embed.width=frame.getAttribute('width')||'600';embed.height=frame.getAttribute('height')||'450';embed.loading=frame.getAttribute('loading')||'lazy';embed.referrerPolicy=frame.getAttribute('referrerpolicy')||'strict-origin-when-cross-origin';embed.allowFullscreen=true;preview.appendChild(embed);});preview.hidden=!maps.length;return maps.length>0;};
    const initializeEditors=()=>root.querySelectorAll('[data-rich-text-component]').forEach(element=>{const editor=new Quill(element,{theme:'snow',modules:{toolbar:[[{header:[1,2,3,false]}],['bold','italic','underline','strike'],[{list:'ordered'},{list:'bullet'}],['blockquote','link'],[{align:[]}],['clean']]}});const fieldElement=element.parentElement;const toolbar=fieldElement.querySelector('.ql-toolbar');element.__shoproQuill=editor;[element,toolbar].filter(Boolean).forEach(editorPart=>editorPart.addEventListener('click',event=>event.stopPropagation()));editor.clipboard.dangerouslyPasteHTML(element.dataset.content||'');if(showEmbedPreview(fieldElement,element.dataset.content||'')){toolbar.hidden=true;element.hidden=true;}editor.on('text-change',()=>{const component=findComponent(element.closest('[data-component-id]').dataset.componentId);if(component){component.data.content=editor.root.innerHTML;const source=fieldElement.querySelector('[data-rich-text-source]');if(!source.hidden)source.value=component.data.content;markDirty();synchronize();}});});
    ...
    list.addEventListener('click',event=>{
        ...
        const columnElement=event.target.closest('[data-column]');if(columnElement&&!event.target.closest('[data-component-id],.builder-column-toolbar')){selectedSlot={sectionId:section.id,column:Number(columnElement.dataset.column)};list.querySelectorAll('.builder-section__column.is-selected').forEach(element=>element.classList.remove('is-selected'));columnElement.classList.add('is-selected');return;}
        ...
        if(event.target.closest('[data-rich-text-source-toggle]')&&component){const button=event.target.closest('[data-rich-text-source-toggle]');const fieldElement=button.closest('.component-rich-field');const editorElement=fieldElement.querySelector('[data-rich-text-component]');const toolbar=fieldElement.querySelector('.ql-toolbar');const preview=fieldElement.querySelector('[data-rich-text-embed-preview]');const source=fieldElement.querySelector('[data-rich-text-source]');const enteringSource=source.hidden;if(enteringSource){source.value=component.data[source.dataset.field]||'';source.hidden=false;toolbar.hidden=true;editorElement.hidden=true;preview.hidden=true;button.textContent=t('Wróć do podglądu','Back to preview');}else{component.data[source.dataset.field]=source.value;source.hidden=true;const hasEmbed=showEmbedPreview(fieldElement,source.value);toolbar.hidden=hasEmbed;editorElement.hidden=hasEmbed;if(!hasEmbed)editorElement.__shoproQuill.clipboard.dangerouslyPasteHTML(source.value,'silent');markDirty();synchronize();button.textContent=t('Edytuj HTML','Edit HTML');}return;}
    ...
    document.addEventListener('shopro:component-options-updated', render);
