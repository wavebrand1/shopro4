...
const initializeComponentBuilder = () => {
    ...
    const field = (definition,value,scope) => {
        ...
        if(definition.type==='multiselect'){const selected=new Set(String(value||'').split(',').map(item=>item.trim()).filter(Boolean));return '<label>'+escape(definition.label)+'<select multiple size="'+escape(definition.size||6)+'"'+attr+'>'+definition.options.map(option=>'<option value="'+escape(option[1])+'"'+(selected.has(String(option[1]))?' selected':'')+'>'+escape(option[0])+'</option>').join('')+'</select>'+(definition.help?'<small>'+escape(definition.help)+'</small>':'')+'</label>';}
