...
const initializeComponentBuilder = () => {
    ...
    const sectionHasSystemRole = section => (section.data.columns || []).some(column => column.some(component => component.type === 'system_role'));
    ...
    list.addEventListener('click',event=>{
        ...
        else if(event.target.closest('[data-remove-section]')){if(sectionHasSystemRole(section))return;blocks.splice(sectionIndex,1);}
        else if(event.target.closest('[data-duplicate-section]')){if(sectionHasSystemRole(section))return;const copy=duplicateSection(section);blocks.splice(sectionIndex+1,0,copy);selectedSlot={sectionId:copy.id,column:0};}
        ...
        else if(event.target.closest('[data-remove-component]')){if(component.type==='system_role')return;column.splice(componentIndex,1);}
