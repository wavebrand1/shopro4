...
const initializeComponentBuilder = () => {
    ...
    const sectionTemplate=(section,index)=>'<details class="builder-section" data-section-id="'+escape(section.id)+'" open><summary class="builder-section__header"><span>Sekcja '+(index+1)+'</span><select data-section-field="container" aria-label="Szerokość sekcji"><option value="grid"'+(section.data.container==='grid'?' selected':'')+'>W gridzie</option><option value="full"'+(section.data.container==='full'?' selected':'')+'>Pełna szerokość</option></select><div class="component-item-actions"><button type="button" data-add-column>+ Kolumna</button><button type="button" data-move-section="-1">↑</button><button type="button" data-move-section="1">↓</button><button class="is-danger" type="button" data-remove-section>Usuń</button></div><b>⌄</b></summary><div class="builder-section__columns" style="grid-template-columns:'+section.data.widths.map(width=>Math.max(1,Number(width)||1)+'fr').join(' ')+'">'+section.data.columns.map((column,columnIndex)=>'<div class="builder-section__column'+(selectedSlot?.sectionId===section.id&&selectedSlot.column===columnIndex?' is-selected':'')+'" data-column="'+columnIndex+'"><div class="builder-column-toolbar"><small>Kolumna '+(columnIndex+1)+'</small><label>Szerokość <input type="number" min="1" max="100" value="'+escape(section.data.widths[columnIndex]||1)+'" data-column-width="'+columnIndex+'">%</label><button type="button" class="is-danger" data-remove-column="'+columnIndex+'"'+(section.data.columns.length===1?' disabled':'')+'>Usuń</button></div>'+column.map(componentTemplate).join('')+'</div>').join('')+'</div></details>';
    ...
    list.addEventListener('click',event=>{
        ...
        if(event.target.closest('[data-add-column]')){section.data.columns.push([]);section.data.widths.push(Math.round(100/section.data.columns.length));selectedSlot={sectionId:section.id,column:section.data.columns.length-1};}
