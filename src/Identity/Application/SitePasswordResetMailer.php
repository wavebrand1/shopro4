...
final class SitePasswordResetMailer
{
    ...
    public function send(SiteUser $user, string $resetUrl): void
    {
        ...
        $variables = ['[SITE_NAME]' => (string) $this->settings->get('site_name', 'Shopro'), '[USERNAME]' => $user->getUsername(), '[EMAIL]' => $user->getEmail(), '[LINK]' => $resetUrl];
        ...
        $email = (new Email())->from(sprintf('%s <%s>', $this->settings->get('mail_from_name', 'Shopro'), $from))->to($user->getEmail())->subject($subject)->text(trim(html_entity_decode(strip_tags($content), ENT_QUOTES | ENT_HTML5, 'UTF-8'))."\n\n".$resetUrl)->html($this->layout->render($content, $subject));
