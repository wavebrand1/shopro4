...
final class UrlRedirectController extends AbstractController
{
    ...
    public function new(Request $request, UrlRedirectRepository $repository): Response { return $this->form($request, new UrlRedirect(), $repository); }
    #[Route('/{id}/edit', name: 'admin_url_redirect_edit', requirements: ['id' => '\d+'], methods: ['GET', 'POST'])]
    public function edit(UrlRedirect $redirect, Request $request, UrlRedirectRepository $repository): Response { return $this->form($request, $redirect, $repository); }
    ...
    private function form(Request $request, UrlRedirect $redirect, UrlRedirectRepository $repository): Response
    {
        $form = $this->createForm(UrlRedirectType::class, $redirect); $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            if ($redirect->getSourcePath() === $redirect->getTargetPath()) $form->addError(new \Symfony\Component\Form\FormError($this->translator->translate('redirect.loop')));
            else { $repository->save($redirect); $this->addFlash('success', $this->translator->translate('redirect.saved')); return $this->redirectToRoute('admin_url_redirect_index'); }
        }
